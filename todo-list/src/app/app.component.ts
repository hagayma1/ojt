...
export class AppComponent {
  title = 'todo-list';
  tasks = backupSaver.restore('tasks'); // CR: "tasks" should be a GLOBAL CONSTANT (file scope/project scope)
  ...
  onTaskAdded(data: { text: string; checked: boolean }) {
    console.log(this.tasks); // CR: always remove all console.log calls
  ...
  filterTasks() {
    this.tasksToShow = this.tasks.filter((task) =>
      task.text.includes(this.filter)
    );
  }
